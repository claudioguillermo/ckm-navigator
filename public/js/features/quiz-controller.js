...
class QuizController {
    ...
    handleQuizAnswer(index) {
        ...
        if (this.app.currentQuizStep < t.questions.length) {
            ...
            this.app.tasks.setTimeout(() => this.showQuizResult(), 300);
        }
    }
    ...
    renderPassport(stage) {
        ...
        DOMUtils.safeSetHTML(container, `
            <div class="passport-modal" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.8); z-index: 9999; display: flex; align-items: center; justify-content: center; padding: 20px;">
                <div class="passport-card animate-slide-up" style="background: var(--bg-card); border-radius: 20px; width: 100%; max-width: 400px; overflow: hidden; color: var(--text-primary); border: 1px solid var(--border-soft);">
                    ...
                    <div style="padding: 24px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--stroke-subtle); padding-bottom: 16px; margin-bottom: 16px;">
                            <div>
                                <div style="font-size: 11px; text-transform: uppercase; color: var(--text-tertiary); font-weight: 700;">${pp.stageStatus || 'Stage Status'}</div>
                            ...
                            <div style="text-align: right;">
                                <div style="font-size: 11px; text-transform: uppercase; color: var(--text-tertiary); font-weight: 700;">${pp.medications || 'Medications'}</div>
                                <div style="font-size: 28px; font-weight: 900;">${safeMedCount}</div>
                            </div>
                        </div>

                        <div style="margin-bottom: 24px;">
                            <div style="font-size: 11px; text-transform: uppercase; color: var(--text-tertiary); font-weight: 700; margin-bottom: 8px;">${pp.clinicalPriorities || 'Clinical Priorities'}</div>
                            <ul style="padding-left: 20px; margin: 0; font-size: 14px; line-height: 1.5;">
                                ${priorityItems.map(item => `<li>${item}</li>`).join('')}
                            </ul>
                        </div>

                        <div style="background: var(--bg-depth); padding: 16px; border-radius: 12px; text-align: center;">
                            <p style="margin: 0; font-size: 12px; color: var(--text-tertiary);">${pp.footerText || 'Show this screen to your doctor to start a conversation about your CKM health goals.'}</p>
                        </div>
                    </div>

                    <button type="button" class="btn btn-secondary" data-action="closePassport" style="width: 100%; padding: 20px; border: none; border-radius: 0 0 20px 20px; font-weight: 700; cursor: pointer;">${pp.closeBtn || 'Close'}</button>
                </div>
            </div>
        `);
