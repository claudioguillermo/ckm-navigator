// CKM Navigator Main Coordinator
...
const app = {
    ...
    async init() {
        ...
        if (typeof TaskManager !== 'undefined') {
            ...
            // Fallback dummy
            this.tasks = { setTimeout: (cb, ms) => setTimeout(cb, ms), abortAll: () => { }, signal: new AbortController().signal };
        }
        ...
        // Handle direct chat link (flag set by checkUrlParams)
        if (this._openChatOnLoad) {
            this.tasks.setTimeout(() => this.toggleChat(true), 800);
        }
    ...
    applyTheme(theme, persist = true) {
        ...
        // Remove existing theme-color meta tags to prevent conflicts particularly with media query tags
    ...
    navigateTo(pageId) {
        ...
        const chatItem = Array.from(this.navItems || []).find(i => i.getAttribute('data-action')?.includes('toggleChat'));
    ...
    async renderFinalReport() {
        if (this.curriculumController && typeof this.curriculumController.renderFinalReport === 'function') {
            return this.curriculumController.renderFinalReport(this);
        }
    },

    submitFeedback() {
        if (this.curriculumController && typeof this.curriculumController.submitFeedback === 'function') {
            return this.curriculumController.submitFeedback(this);
        }
    },
...
`;
    ...
    initChatInteraction() {
        return this.chatController.initChatInteraction();
    }
};
